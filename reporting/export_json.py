...
def write_markdown_summary(output_dir: Path, summary: dict) -> Path:
    ...
    lines += [
        ...
        "| Phase | avg | p50 | p95 |",
        "|-------|-----|-----|-----|",
        f"| Dispatcher queue | {_sec(wf.get('dispatcher_queue_avg'))} | — | {_sec(wf.get('dispatcher_queue_p95'))} |",
        f"| Dispatcher run | {_sec(wf.get('dispatcher_duration_avg'))} | — | {_sec(wf.get('dispatcher_duration_p95'))} |",
        f"| Pipeline gap (Kafka → omnipass) | {_sec(wf.get('pipeline_gap_avg'))} | — | {_sec(wf.get('pipeline_gap_p95'))} |",
        f"| Omnipass queue (semaphore wait) | {_sec(wf.get('omnipass_queue_avg'))} | — | {_sec(wf.get('omnipass_queue_p95'))} |",
        f"| Omnipass run (calrissian) | {_sec(wf.get('omnipass_duration_avg'))} | — | {_sec(wf.get('omnipass_duration_p95'))} |",
        f"| STAC publish latency | {_sec(biz.get('stac_latency_avg'))} | — | {_sec(biz.get('stac_latency_p95'))} |",
        f"| Gap to deletion (omnipass→delete) | {_sec(wf.get('gap_to_deletion_avg'))} | — | {_sec(wf.get('gap_to_deletion_p95'))} |",
        f"| Deletion queue | {_sec(wf.get('deletion_queue_avg'))} | — | {_sec(wf.get('deletion_queue_p95'))} |",
        f"| Deletion run | {_sec(wf.get('deletion_duration_avg'))} | — | {_sec(wf.get('deletion_duration_p95'))} |",
        f"| **End-to-end total** | **{_sec(biz.get('e2e_avg'))}** | **{_sec(biz.get('e2e_p50'))}** | **{_sec(biz.get('e2e_p95'))}** |",
        "",
        "_T0 = `ingest_reference_time` (drop-bucket event or dispatcher created_at)._  ",
        "_T_final = `deletion_finished_at` (falls back to `stac_seen_at` when deletion not observed)._",
        "",
    ]
